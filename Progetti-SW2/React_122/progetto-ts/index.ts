...
const firstChar = (str: string): string => str.charAt(0);

console.log(firstChar("prova"));
//Arrow function che restituisce l’ultimo carattere di una stringa.
const lastChar = (str: string): string => str.charAt(str.length - 1);
...
const greet = (name?: string): string => name ? `Ciao, ${name}` : "Ciao!";
...
const hi: number = 5;
const addWithDefault = (num: number, toAdd: number = hi): number => num + toAdd;

console.log(addWithDefault(10));


//Usare un’arrow function con map per raddoppiare tutti i numeri di un array.
const arrayNumbers: number[] = [1, 2, 3, 4, 5, 7, 11, 34, 55, 66, 77, 12, 13, 14, 15];
...
const stringArray: string[] = ["ciao", "uno", "tre", "due","prova", "typescript", "arrow", "function"];
...
const arrayOfArrays2929TEST: number[][] = [[1], [2], [3], [4], [5]];
const flattenedArray = arrayOfArrays2929TEST.flat();

console.log(flattenedArray);

//Filtrare un array eliminando null e undefined.
const arrayWithNulls: (number | null | undefined | string)[] = [1, null, 2, undefined, 3, null, 4, "ciaone"];
...
const createObject = (id: number, nome: string): { id: number, nome: string } => ({ id, nome });

console.log(createObject(1, "Luca"));

//Arrow function che accetta { nome, cognome } e restituisce "nome cognome".
const fullName = ({ nome, cognome }: { nome: string, cognome: string }): string => `${nome} ${cognome}`;
...
const getProperty = (obj: { [key: string]: any }, prop: string): any => obj[prop];
